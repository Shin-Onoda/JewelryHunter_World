...
### 作品について
Unityで2Dゲームを学ぶにあたって、基本的な機能が網羅されている作品となるように頑張りました。ワールドマップ上で好きなステージのロックを解除して挑戦できることで、プレイする度に異なる遊び方・攻略の仕方ができるように工夫しました。
...
* 決定：Spaceキー/Enterキー/Eastボタン（ゲームパッド）

ステージ中
* 移動：ADキー/左右キー/左スティック（ゲームパッド）
* 矢を打つ：Enterキー/Westボタン（ゲームパッド）
...
2Dサイドのアクションと、見下ろし型のトプビューという異なるゲーム性のパートを矛盾が無いようにブレンドしています。
* アクションパート専用のUIとPlayerスクリプト
* ワールドマップ専用のUIとPlayerスクリプト
* 共通するGameManagerやSoundManager

### セーブ機能の実装
ゲームの一定の状態をオートセーブして続きから遊べるように配慮しました。PlayerPrefsとJSONをうまく活用しています。
...
// JsonUtility で扱うためのシリアライズ可したクラス
[System.Serializable] 
public class SaveData
{
   //略
}
//シリアライズ可したSaveDataクラスを扱っていくマネージャー
...
## 課題
今回はUnityの技術を磨くための作品としてある程度書籍を参考に作り始めました。書籍の内容と自分のアレンジとツギハギした形となりましたので、ゲームステータスのコントロールや、UIのコントロールが無理やりな部分があったと思います。
